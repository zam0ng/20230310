...
// UTXO
// 각 노드의 UTXO 데이터베이스
// 각 주소별로 가지고있는 잔액 금액을 가지고 있는 객체의 내용이 포함 되어있다.

// A가 B에게 1비트를 보내면
// 트랜잭션을 발생 시키고
// txIn에 이전 트랜잭션에서 남은 미사용 객체 UTXO를 참조해서(A가 얼마가지고잇는지 확인 잔액들)
...
class Unspent {
    ...
    // 미사용 객체를 txin에서 참조할때 객체를 조회하는데
    // 사용하고나면 객체를 값을 수정하는게 아니라 한번도 사용하지않는 객체들이 UTXO에 담겨있고
    // 사용을하면 제거
    delate(txin : TxIn) {
    ...
    update(transaction : TransactionRow) {
        ...
        if(!hash) throw new Error("Hash가 정상이지 않다.");
    ...
    getAmount(myUnspentTxOuts : UnspentTxOut[]) {
        ...
        // 쿨백의 두번째 매개변수는 현재 순회하는 요소
        // 값의 합을 내보낸것
        return myUnspentTxOuts.reduce((acc,utxo)=>acc + utxo.amount, 0)
    }

    // 주어진 계정의 잔고가 보내는 금액보다 큰지 검증
    isAmount(accout : string, sendAmount : number){
        // 내 주소와 잔액 정보가 있는 사용하지않은 객체 조회(배열)
        const myUnspentTxOuts = this.getUTXO(accout);
        // totalAmount 총 잔액
